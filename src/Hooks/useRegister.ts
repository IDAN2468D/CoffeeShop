...
const useRegister = () => {
    ...
    const checkUserExistence = async (values: FormValues) => {
        try {
            const response = await axios.get<{ exists: boolean }>('https://coffeeshopbackend-production-af44.up.railway.app/user-exist', {
                params: {
                    email: values.email,
            ...
            if (!response.data.exists) {
                const registerResponse = await axios.post<{ user: any }>('https://coffeeshopbackend-production-af44.up.railway.app/register', {
                    email: values.email,
                    name: values.userName,
